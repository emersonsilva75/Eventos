...
def validate_livros(livro):
    if is_positivo(livro):
        livros = formataLista(obtemLivros(cultural=1),'titulo')
        return build_validation_result(False, 'slotOne', 'Segue a relação de livros : \n {}'.decode("ISO-8859-1").format(livros) )
...
def gravarBanco(tabela,item):
    try:
        dynamodb_client.put_item(TableName=tabela, Item=item)
    except Exception as e:
        print(traceback.format_exc())

def ordenaJsonPorTitulo(contexto):
    return contexto['titulo']

def ordenaJsonPorLink(contexto):
    return contexto['link']

def formataLista(conteudo,campo):
    html = ''
    conteudoOrdenado =  sorted(conteudo,key=ordenaJsonPorTitulo)
    for contem in conteudoOrdenado:
        html = html + '\n {}'.format(contem[campo]).decode("ISO-8859-1")
    return html

def obtemLivros(cultural = None):
    try:
        table = dynamodb.Table('Livros')
        if cultural is not None :
                response = table.scan(
                    FilterExpression=Attr('cultural').eq(1)
                    )
        if cultural is None :
            response = table.scan()
